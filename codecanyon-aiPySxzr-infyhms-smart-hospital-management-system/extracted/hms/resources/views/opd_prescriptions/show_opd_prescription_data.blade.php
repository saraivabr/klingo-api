...
<div class="row">
    <div class="form-group col-sm-12 my-0">
        <a href="{{ route('opdPrescription.pdf', $opdPrescription->id) }}" class="btn btn-sm btn-success {{getCurrentLoginUserLanguageName() == 'ar' ? 'float-start' : 'float-end'}}" target="_blank">
...
<div id='DivIdToPrint'>
    ...
    <div class="row g-5 mb-11">
        <div class="col-sm-2">
            <div class="fw-bold {{getCurrentLoginUserLanguageName() == 'ar' ? 'ps-10' : 'pe-10'}} text-gray-600 fs-7">{{ __('messages.opd_patient.opd_number') . ':' }}</div>
        ...
        <div class="col-sm-2">
            ...
            <div class="fw-bolder fs-6 text-gray-800 text-break">{{ $opdPrescription->patient->patient->user->email }}
            </div>
        </div>
        <div class="col-sm-2">
    ...
    <div class="row g-5 mb-11">
        <div class="col-sm-2 ">
            <div class="fw-bold {{getCurrentLoginUserLanguageName() == 'ar' ? 'ps-10' : 'pe-10'}} text-gray-600 fs-7">{{ __('messages.opd_patient.appointment_date') . ':' }}</div>
            <div class="fw-bolder fs-6 text-gray-800">
                {{ date('jS M, Y H:i', strtotime($opdPrescription->patient->appointment_date)) }}</div>
        </div>
        <div class="col-sm-2 ">
    ...
    <div class="row g-5 mb-11">
        <div class="col-sm-12">
            ...
            <div class="fw-bolder fs-6 text-gray-800">{!! $opdPrescription->patient->symptoms != null
                ? $opdPrescription->patient->symptoms
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer w-100 mb-3">
            ...
            <tbody class="text-gray-600 fw-bold">
                ...
                <tr class="p-0">
                    ...
                    <tr class="fw-bolder text-gray-700 fs-5 t{{getCurrentLoginUserLanguageName() == 'ar' ? 'text-start' : 'text-end'}}">
                        <td class="{{getCurrentLoginUserLanguageName() == 'ar' ? 'text-end' : 'text-start'}} pt-6">{{ $opdPrescriptionItem->medicineCategory->name }}
                            - {{ $loop->iteration }}</td>
                        <td class="{{getCurrentLoginUserLanguageName() == 'ar' ? 'text-end' : 'text-start'}} pt-6">{{ $opdPrescriptionItem->medicine->name }}</td>
                        ...
                        <td class="{{getCurrentLoginUserLanguageName() == 'ar' ? 'text-end' : 'text-start'}} pt-6">{!! !empty($opdPrescriptionItem->instruction)
