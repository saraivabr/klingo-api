        ...
        <div class="dropdown-menu py-0" aria-labelledby="dropdownMenuButton1">
            ...
            <div class="p-5">
                <div class="mb-5">
                    ...
                    {{-- {{ Form::select('status', collect($filterHeads[0])->sortBy('key')->toArray(), null, ['id' => 'issuedItemHead', 'data-control' =>'select2', 'class' => 'form-select status-filter']) }} --}}
                    <select class="form-select status-selector " id="issuedItemHead" data-control="select2"
                        name="status">
                        <option value="2">{{ __('messages.filter.All') }}</option>
                        <option value="0">{{ __('messages.issued_item.item_return') }}</option>
                        <option value="1">{{ __('messages.issued_item.item_returned') }}</option>
                    </select>
                </div>
