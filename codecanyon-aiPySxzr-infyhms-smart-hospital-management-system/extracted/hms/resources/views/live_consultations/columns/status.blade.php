        ...
        <div class="w-150px d-flex align-items-center">
            ...
            <select class="form-select change-consultation-status"
                ...
                <option value="0" {{ $row->status == 0 ? 'selected' : '' }}{{ ($row->status == 1 || $row->status == 2)  ? 'disabled' : '' }}>
                    {{__('messages.live_consultation.awaited')}}
                </option>
                <option value="1" {{ $row->status == 1 ? 'selected' : '' }}
                        {{ ($row->status == 2)  ? 'disabled' : '' }}>{{__('messages.live_consultation.cancelled')}}
                </option>
                <option value="2" {{$row->status == 2 ? 'selected' : ''}}
                        {{ ($row->status  == 1)  ? 'disabled' : '' }}>{{__('messages.live_consultation.finished')}}
                </option>
            </select>
        </div>
    @elseif ($row->status == 1)
...
@else
    @if ($row->status == 1)
        <span class="badge bg-light-danger ms-2 fs-8 py-1 px-3">{{__('messages.live_consultation.cancelled')}}</span>
    @elseif($row->status == 0)
        <span class="badge bg-light-warning ms-2 fs-8 py-1 px-3">       {{__('messages.live_consultation.awaited')}}</span>
    @elseif ($row->status == 2)
        <span class="badge bg-light-success ms-2 fs-8 py-1 px-3"> {{__('messages.live_consultation.finished')}}</span>
    @else
        {{ $row->status_text }}
