        ...
        <div class="row">
            <div class="col-lg-5 col-md-6">
                <div class="d-flex align-items-center mb-md-10 mb-5">
                    ...
                    <h3 class="{{getCurrentLoginUserLanguageName()=='ar' ? 'pe-7' : 'ps-7'}}">{{ __('messages.invoice.invoice') }} #{{ $invoice->invoice_id }}</h3>
                </div>
            </div>
            <div class="col-lg-4 col-md-3 col-6">
                <div class="d-flex flex-column mb-md-10 mb-5 mt-3 mt-md-0">
                    ...
                    <span
                        class="fs-5 text-gray-800">{{ \Carbon\Carbon::parse($invoice->invoice_date)->translatedFormat('jS M, Y') }}</span>
                </div>
            </div>
            ...
            <div class="col-lg-5 col-md-6">
                <div class="d-flex flex-column mb-md-10 mb-5">
                    <label for="name" class="pb-2 fs-5 text-gray-600">{{ __('messages.invoice.issue_for') }}
                        :</label>
            ...
            <div class="col-md-2 col-md-6">
                <div class="d-flex flex-column mb-md-10 mb-5">
                    <label for="name" class="pb-2 fs-5 text-gray-600">{{ __('messages.invoice.issue_from') }}
                        :</label>
                ...
                <table class="table table-striped box-shadow-none mt-4">
                    <thead>
                        <tr class="border-bottom fs-6  text-muted">
                            ...
                            <th class="{{getCurrentLoginUserLanguageName()=='ar' ? 'text-start' : 'text-end'}} pb-2">{{ __('messages.invoice.qty') }}</th>
                            <th class="{{getCurrentLoginUserLanguageName()=='ar' ? 'text-start' : 'text-end'}} pb-2">{{ __('messages.invoice.price') }}</th>
                            <th class="{{getCurrentLoginUserLanguageName()=='ar' ? 'text-start' : 'text-end'}} pb-2">{{ __('messages.invoice.amount') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($invoice->invoiceItems as $index => $invoiceItem)
                            <tr class="{{getCurrentLoginUserLanguageName()=='ar' ? 'text-start' : 'text-end'}}">
                                <td class="pt-6 {{getCurrentLoginUserLanguageName()=='ar' ? 'text-end' : 'text-start'}}">{{ $invoiceItem->account->name }}</td>
                                <td class="pt-6 {{getCurrentLoginUserLanguageName()=='ar' ? 'text-end' : 'text-start'}}">{!! $invoiceItem->description != '' ? nl2br(e($invoiceItem->description)) : __('messages.common.n/a') !!}</td>
                                <td class="pt-6">{{ $invoiceItem->quantity }}</td>
                                <td class="pt-6">
                                    {{ checkNumberFormat($invoiceItem->price, strtoupper(getCurrentCurrency())) }}
                                </td>
                                <td class="pt-6 text-dark">
                                    {{ checkNumberFormat($invoiceItem->total, strtoupper(getCurrentCurrency())) }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="col-lg-5  {{getCurrentLoginUserLanguageName() == 'ar' ? 'me-lg-auto' : 'ms-lg-auto'}} mt-4">
                <div class="border-top">
                    <table
                        class="table table-borderless {{ getLoggedInUser()->thememode ? '' : 'bg-white' }} box-shadow-none mb-0 mt-5">
                        <tbody class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }}">
                            <tr>
                                <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} ps-0">
                                    {{ __('messages.invoice.sub_total') . ':' }}</td>
                                <td
                                    class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} text-gray-900 {{getCurrentLoginUserLanguageName()=='ar' ? 'text-start' : 'text-end'}} pe-0">
                            ...
                            <tr>
                                ...
                                <td
                                    class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} text-gray-900 {{getCurrentLoginUserLanguageName()=='ar' ? 'text-start' : 'text-end'}} pe-0">
                                    {{ checkNumberFormat(($invoice->amount * $invoice->discount) / 100, strtoupper(getCurrentCurrency())) }}
                                </td>
                            </tr>
                            <tr>
                                <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} ps-0">
                                ...
                                <td
                                    class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} text-gray-900 {{getCurrentLoginUserLanguageName()=='ar' ? 'text-start' : 'text-end'}} pe-0">
                                    {{ checkNumberFormat($invoice->amount - ($invoice->amount * $invoice->discount) / 100, strtoupper(getCurrentCurrency())) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <div class="bg-light-100 rounded-15 p-md-7 p-5 h-100 mt-xxl-0 mt-5 col-xxl-9 ms-xxl-auto">
            <div class="mb-8">
                @if ($invoice->status == \App\Models\Invoice::PENDING)
                    <span class="badge bg-light-warning">{{ __('messages.appointment.pending') }}</span>
                @elseif($invoice->status == \App\Models\Invoice::PAID)
                    <span class="badge bg-light-success me-2">{{ __('messages.invoice.paid') }}</span>
                @endif
            </div>
            <h6 class="mb-5">{{ __('messages.invoice.patient_overview') }}</h6>
            <div class="mb-6">
