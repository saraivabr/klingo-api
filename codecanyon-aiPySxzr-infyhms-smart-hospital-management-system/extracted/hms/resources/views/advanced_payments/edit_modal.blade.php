            ...
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <div class="form-group col-sm-12 mb-5">
                        ...
                        <span class="required"></span>
                        {{ Form::text('receipt_no', null, ['class' => 'form-control', 'id' => 'editReceiptNo', 'required', 'readonly', 'placeholder' => __('messages.advanced_payment.receipt_no')]) }}
                    </div>
                    <div class="form-group col-sm-12 mb-5">
                        {{ Form::label('amount', __('messages.advanced_payment.amount') . ':', ['class' => 'form-label required']) }}
                        {{ Form::text('amount', null, ['class' => 'form-control price-input', 'id' => 'editAmount', 'required', 'onkeyup' => 'if (parseInt(this.value.replace(/[^\d.]/g, "")) > 1000000) this.value = this.value.slice(0, -1)', 'placeholder' => __('messages.advanced_payment.amount')]) }}
                    </div>
                    <div class="form-group col-sm-12 mb-5">
                        {{ Form::label('date', __('messages.advanced_payment.date') . ':', ['class' => 'form-label required']) }}
                        {{ Form::text('date', null, ['class' => getLoggedInUser()->thememode ? 'bg-light form-control' : 'bg-white form-control', 'id' => 'editAdvancedPaymentDate', 'required', 'autocomplete' => 'off', 'placeholder' => __('messages.advanced_payment.date')]) }}
                    </div>
                </div>
            </div>
