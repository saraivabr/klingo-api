    ...
    <div class="col-md-2">
        <div class="mb-5">
            <div class="mb-5">
                ...
                {{ Form::select('case_id', [null], null, ['class' => 'form-select', 'required', 'id' => 'opdCaseId', 'disabled', 'data-control' => 'select2', 'placeholder' => __('messages.common.choose') . __('messages.cases')]) }}
            </div>
        </div>
    </div>
    <div class="col-md-2">
    ...
    <div class="col-md-2">
        <div class="mb-5">
            <div class="mb-5">
                ...
                {{ Form::number('height', isset($data['last_visit']) ? $data['last_visit']->height : 0, ['placeholder' => __('messages.ipd_patient.height'), 'class' => 'form-control',  'step' => '.01']) }}
            </div>
        </div>
    </div>
    <div class="col-md-2">
    ...
    <div class="col-md-2">
        <div class="mb-5">
            <div class="mb-5">
                ...
                {{ Form::number('bp', isset($data['last_visit']) ? $data['last_visit']->bp : null, ['class' => 'form-control', 'placeholder' => __('messages.ipd_patient.bp')]) }}
            </div>
        </div>
    </div>
    <div class="col-md-3">
    ...
    <div class="col-md-3">
        <div class="mb-5">
            <div class="mb-5">
                ...
                <span class="required"></span>
                {{ Form::select('doctor_id', $data['doctors'], isset($data['last_visit']) ? $data['last_visit']->doctor_id : null, ['placeholder' => __('messages.ipd_patient.doctor_id'), 'class' => 'form-select', 'required', 'id' => 'opdDoctorId', 'placeholder' => __('messages.web_home.select_doctor'), 'data-control' => 'select2']) }}
            </div>
        </div>
    </div>
    <div class="col-md-3">
        ...
        @foreach ($customField as $field)
            ...
            <div class="form-group col-sm-{{$field['grid']}} mb-5 text-capitalize">
                ...
                @elseif ($field_type == 'multiSelect' )
                    {{ Form::select($fieldName.'[]', $field_values, 0, ['class' => $field['is_required'] == 1 ? 'form-select opd-multi-select dynamic-field' : 'form-select opd-multi-select', 'placeholder' => $field['field_name'], 'data-control' => 'select2','multiple' => true,$field['is_required'] == 1 ? 'required' : '']) }}
                @else
                    {{ Form::$field_type($fieldName, null, ['class' => $field['is_required'] == 1 ? 'dynamic-field form-control' : 'form-control', 'placeholder' => $field['field_name'],$field['is_required'] == 1 ? 'required' : '','rows' => $field_type == 'textarea' ?  4 : '']) }}
                @endif
            </div>
        @endforeach
    @endif
...
<div class="d-flex justify-content-end">
    {!! Form::submit(__('messages.common.save'), ['class' => getCurrentLoginUserLanguageName()== 'ar' ? 'btn btn-primary  ms-3' : 'btn btn-primary me-3', 'id' => 'btnOpdSave']) !!}
